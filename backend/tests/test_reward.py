...
def test_reward_assignment_on_approval(client):
    ...
    # 1. ユーザー作成
    parent_resp = client.post("/api/auth/register", json={"name": "P", "role": "parent", "pin": "1234"})
    child_resp = client.post("/api/auth/register", json={"name": "C", "role": "child", "pin": "1234"})
...
def test_wallet_initial_balance(client):
    ...
    resp = client.post("/api/auth/register", json={"name": "Zero", "role": "child", "pin": "0000"})
